...
public class Solution {
    ...
    public TreeNode balanceBST(TreeNode root) {
        ...
        int n = inorder.size();
        return balanceTree(0, n - 1, inorder);
    }

    private void inorderList(TreeNode root, List<Long> inorder) {
        if (root != null) {
    ...
    private TreeNode balanceTree(int start, int end, List<Long> list) {
        if (start > end) {
            return null;
        }
        int mid = (start + end) / 2;
        TreeNode root = new TreeNode(Math.toIntExact(list.get(mid)));
