...
class FractionalKnapsack{
    ...
    {   
        double totalCost=0;
        PriorityQueue<ItemValue> pq=new PriorityQueue<>(
            (a,b)->{
        ...
        for(int i=0;i<n;i++){
            pq.offer(new ItemValue(wt[i],val[i],i));
        }
        while(capacity>0 && !pq.isEmpty()){
            ItemValue item=pq.poll();
            double value=item.wt;
            double having=Math.min(value,capacity);
            capacity-=(int)having;
            totalCost+=having*item.cost;
        }
        return totalCost;
